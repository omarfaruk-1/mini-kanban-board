...
export default function BoardMembers({ board, isOwner, onChanged }) {
  ...
  async function add(e) {
    ...
    setLoading(true);
    try {
      await memberService.add(board.id, email.trim());
      setEmail("");
  ...
  async function remove(id) {
    try {
      await memberService.remove(board.id, id);
  ...
  return (
    <div>
      {isOwner && (
        <form onSubmit={add} className="mb-6 rounded-xl bg-slate-50 p-4">
